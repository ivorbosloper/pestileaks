# -*- coding: utf-8 -*-
import datetime
from south.db import db
from south.v2 import SchemaMigration
from django.db import models


class Migration(SchemaMigration):

    def forwards(self, orm):

        # Changing field 'GebruiksRegels.veiligheidstermijn'
        db.alter_column('pestileaks_gebruiksregels', 'veiligheidstermijn', self.gf('django.db.models.fields.IntegerField')(null=True))

        # Changing field 'GebruiksRegels.wachttijd_betreding'
        db.alter_column('pestileaks_gebruiksregels', 'wachttijd_betreding', self.gf('django.db.models.fields.IntegerField')(null=True))

        # Changing field 'GebruiksRegels.dosering_ondergrens'
        db.alter_column('pestileaks_gebruiksregels', 'dosering_ondergrens', self.gf('django.db.models.fields.FloatField')(null=True))

        # Changing field 'GebruiksRegels.dosering_bovengrens'
        db.alter_column('pestileaks_gebruiksregels', 'dosering_bovengrens', self.gf('django.db.models.fields.FloatField')(null=True))

    def backwards(self, orm):

        # Changing field 'GebruiksRegels.veiligheidstermijn'
        db.alter_column('pestileaks_gebruiksregels', 'veiligheidstermijn', self.gf('django.db.models.fields.IntegerField')(default=0))

        # Changing field 'GebruiksRegels.wachttijd_betreding'
        db.alter_column('pestileaks_gebruiksregels', 'wachttijd_betreding', self.gf('django.db.models.fields.IntegerField')(default=0))

        # Changing field 'GebruiksRegels.dosering_ondergrens'
        db.alter_column('pestileaks_gebruiksregels', 'dosering_ondergrens', self.gf('django.db.models.fields.FloatField')(default=0))

        # Changing field 'GebruiksRegels.dosering_bovengrens'
        db.alter_column('pestileaks_gebruiksregels', 'dosering_bovengrens', self.gf('django.db.models.fields.FloatField')(default=0))

    models = {
        'pestileaks.aantasting': {
            'Meta': {'ordering': "['naam']", 'object_name': 'Aantasting'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'naam': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '50'})
        },
        'pestileaks.gebruiksregels': {
            'Meta': {'object_name': 'GebruiksRegels'},
            'aantasting': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['pestileaks.Aantasting']"}),
            'dosering_bovengrens': ('django.db.models.fields.FloatField', [], {'null': 'True', 'blank': 'True'}),
            'dosering_ondergrens': ('django.db.models.fields.FloatField', [], {'null': 'True', 'blank': 'True'}),
            'gewas': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['pestileaks.Gewas']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'middel': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['pestileaks.Middel']"}),
            'toepassings_methode': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['pestileaks.ToepassingsMethode']"}),
            'veiligheidstermijn': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'}),
            'wachttijd_betreding': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'})
        },
        'pestileaks.gewas': {
            'Meta': {'ordering': "['naam']", 'object_name': 'Gewas'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'naam': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '50'})
        },
        'pestileaks.middel': {
            'Meta': {'ordering': "['naam']", 'object_name': 'Middel'},
            'bedrijf': ('django.db.models.fields.CharField', [], {'max_length': '50', 'blank': 'True'}),
            'eenheid': ('django.db.models.fields.CharField', [], {'max_length': '10', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'naam': ('django.db.models.fields.CharField', [], {'max_length': '50'}),
            'toelatings_nummer': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '10'})
        },
        'pestileaks.toepassingsmethode': {
            'Meta': {'object_name': 'ToepassingsMethode'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'naam': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '50'})
        }
    }

    complete_apps = ['pestileaks']