...
## ISSUES:
## 03992 sla 'ter plaatse gezaaid' is onbekende classificatie
## 04372 Freesia is onbekend gewas

gewas_correctie_lijst = {'freesia':None, 'fresia':None, 'sla (lactuce spp)':'sla (lactuca spp.)', "lactuca spp.":'sla (lactuca spp.)'}
gewas_stadia = {'ongeklapt', 'geklapt'}
...
class Command(BaseCommand):
    def _get_gewas_teeltdoel(self, toepassingsgebied):
        gewas_naam, teeltdoel, filters = toepassingsgebied, None, {}

        m = re.match("(.*) \(((?:[^()])*bedekte teelt)\)",gewas_naam)
        if m:
            filters['bedekt'] = m.group(2)
            gewas_naam = m.group(1)

        m = re.match("(.*) \(met uitzondering van ([^()])+\)",gewas_naam)
        if m:
            filters['uitgezonderd'] = m.group(2)
            gewas_naam = m.group(1)
        
        if gewas_naam.endswith(' ter plaatse gezaaid'):
            gewas_naam = gewas_naam[:-len(' ter plaatse gezaaid')]

        m = re.match("(\w+) \((\w+)\)",gewas_naam)
        if m:
            tussen_haakjes = m.group(2)
            if tussen_haakjes in gewas_stadia:
                filters['stadium'] = tussen_haakjes
            else:
                teeltdoel = TeeltDoel.objects.get(naam__iexact=tussen_haakjes)
            gewas_naam = m.group(1)

        m = re.match("(\w+)\-?aardappelen", gewas_naam)
        if m:
            doel = m.group(1)
            if doel == 'Poot': doel = 'Pootgoed'
            gewas_naam, teeltdoel = "Aardappelen", TeeltDoel.objects.get(naam__iexact=doel)

        gewas_list = Gewas.objects.filter(edi_naam__iexact=gewas_naam).order_by('niveau')
        if gewas_list:
            gewassen = [gewas_list[0]]
            return gewassen, teeltdoel, filters

        gewas_naam = gewas_naam.replace('-', '')

        m = re.match("(\w+)zaad(?:teelt)?",gewas_naam)
        if m:
            if not m.group(1) in ['Winterkool', 'WinterRaap']:
                teeltdoel = TeeltDoel.objects.get(naam__iexact="zaadteelt")
                gewas_naam = m.group(1)

        m = re.match("(Winter|Zomer)(.*)", gewas_naam)
        if m:
            print m.group(1), m.group(2)
            gewas_list = Gewas.objects.filter(edi_naam__iexact=m.group(2)).order_by('niveau')
            if gewas_list:
                filters['periode'] = m.group(1)
                gewassen = [gewas_list[0]]
                return gewassen, teeltdoel, filters
        ...
        if gewas_list:
            ...
            gewassen = []
            for gewas_split_naam in re.split(' en |, ', gewas_naam):
                gewas = Gewas.objects.filter(edi_naam__iexact=gewas_split_naam).order_by('niveau')
                if not gewas:
                    if gewas_split_naam.lower() in gewas_correctie_lijst:
                        if gewas_correctie_lijst[gewas_split_naam.lower()] != None:
                            gewassen.append(Gewas.objects.filter(edi_naam__iexact=gewas_correctie_lijst[gewas_split_naam.lower()]).order_by('niveau')[0])
                        continue
                    raise Exception("Kan gewas %s (uit %s) niet vinden" % (gewas_split_naam, gewas_naam))
                else: gewassen.append(gewas[0]) 
        return gewassen, teeltdoel, filters
...
#                        print "f%s: %s" % (nr, keys)
                    ...
                    gewassen, teeltdoel, filters = self._get_gewas_teeltdoel(mapping['Toepassingsgebied'])
                    ...
                    toepassings_methode,c = ToepassingsMethode.objects.get_or_create(naam=toepassings_methode_naam) if toepassings_methode_naam else (None, False)
                    for gewas in gewassen:
                        GebruiksRegel.objects.get_or_create(gewas=gewas, teeltdoel=teeltdoel, middel=middel, filters=json.dumps(filters,sort_keys=True),
